...
def estatisticas_cooperado():
    ...
    estatisticas = {'total': total, 'pagas': pagas, 'pendentes': pendentes, 'total_valor': total_valor}
...
def excluir_cooperado(id):
    ...
    entregas = Entrega.query.filter_by(cooperado_id=id).all()
...
def exportar_xlsx():
    ...
    for e in entregas:
        dados.append({
            ...
            'Bairro': e.bairro,
            'Valor': e.valor,
            'Data Envio': to_brasilia(e.data_envio).strftime('%d/%m/%Y %H:%M:%S') if e.data_envio else '',
            'Data Recebido': to_brasilia(e.data_recebido).strftime('%d/%m/%Y %H:%M:%S') if e.data_recebido else '',
            'Status': e.status,
            'Cooperado': cooperados.get(e.cooperado_id, '') if e.cooperado_id else '',
