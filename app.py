...
# (Opcional) Admin fixo coopex com duas senhas
ADMIN_CREDENTIALS = {
    'coopex': {
...
class Entrega(db.Model):
    id = db.Column(db.Integer, primary_key=True)
    # identificação do cliente “livre” + vínculo opcional
    ...
    status = db.Column(db.String(20), nullable=True)             # pendente|entregue|recebido
    pagamento = db.Column(db.String(50), nullable=False)         # Pix|Dinheiro|Crédito automático|...
    recebido_por = db.Column(db.String(100), nullable=True)

    # Controle de crédito nesta entrega
...
class CreditoMovimento(db.Model):
    """
    tipo='credito'  (entrada: quando a supervisão concede crédito/ajuste/estorno)
...
# ====== feriados ======
MUNICIPAIS_NATAL = {(11, 21): "Nossa Senhora da Apresentação (Natal/RN)"}

def verifica_feriado(data_ref=None):
    if data_ref is None:
        data_ref = datetime.now(BRAZIL_TZ).date()
    fer_nac = holidays.Brazil(years=data_ref.year)
    fer_est = holidays.Brazil(state='RN', years=data_ref.year)
    nomes = []
    if data_ref in fer_nac: nomes.append(f"Feriado Nacional – {fer_nac.get(data_ref)}")
    if data_ref in fer_est and fer_est.get(data_ref) != fer_nac.get(data_ref):
        nomes.append(f"Feriado Estadual (RN) – {fer_est.get(data_ref)}")
    if (data_ref.month, data_ref.day) in MUNICIPAIS_NATAL:
        nomes.append(f"Feriado Municipal – {MUNICIPAIS_NATAL[(data_ref.month, data_ref.day)]}")
...
def periodo_legivel_str(di_str, df_str):
    if di_str and df_str:
        ...
        return f"{di} a {df}"
    if di_str:
        return f"desde {datetime.strptime(di_str, '%Y-%m-%d').strftime('%d/%m/%Y')}"
    if df_str:
        return f"até {datetime.strptime(df_str, '%Y-%m-%d').strftime('%d/%m/%Y')}"
...
def calcular_valor_final(valor_bruto, desconto_tipo, desconto_valor) -> Decimal:
    ...
    if (desconto_tipo or "nenhum") == "percentual":
...
def pagamento_usa_credito(pagamento: str) -> bool:
    """
    Defina aqui as formas de pagamento que DEVEM usar saldo automaticamente.
    Ex.: 'Crédito automático', 'Crédito', 'Saldo do cliente'
    """
    p = (pagamento or "").strip().lower()
    return p in {"crédito automático", "credito automatico", "credito", "crédito", "saldo do cliente"}
...
def registrar_movimento(cliente_id, tipo, valor, referencia='', credito_id=None, entrega_id=None):
    tipo_up = (tipo or '').lower()
    tm = 'credito' if tipo_up in ('entrada','ajuste','credito','crédito') else 'debito'
    mov = CreditoMovimento(
        cliente_id=cliente_id, tipo=tm, valor=float(_as_decimal(valor)),
        referencia=(referencia or '')[:120], credito_id=credito_id, entrega_id=entrega_id
    )
    db.session.add(mov)
    return mov
...
def registrar_credito(cliente_id:int, valor_bruto, desconto_tipo:str, desconto_valor, motivo:str, criado_por:str):
    ...
    saldo_antes = float(cli.saldo_atual or 0.0)
    valor_final = float(calcular_valor_final(valor_bruto, (desconto_tipo or 'nenhum'), desconto_valor))
    cli.saldo_atual = float(_as_decimal(cli.saldo_atual) + _as_decimal(valor_final))
    cred = Credito(
        cliente_id=cli.id,
        ...
        saldo_depois=float(cli.saldo_atual),
        criado_por=(criado_por or 'Supervisor')
    )
    db.session.add(cred)
    db.session.flush()
    registrar_movimento(cli.id, 'entrada', valor_final, referencia=f'Crédito #{cred.id}', credito_id=cred.id)
    db.session.commit()
    return cred

def consumir_credito_em_entrega(entrega_id: int) -> Decimal:
    """
    Debita automaticamente do saldo do cliente e marca entrega.pago
    quando cobrir 100% (ou parcial se insuficiente).
    """
    e = Entrega.query.get(entrega_id)
    if not e: return Decimal("0.00")

    # cliente pelo id (preferência) ou pelo nome
    cli = Cliente.query.get(e.cliente_id) if getattr(e, "cliente_id", None) else _find_cliente_by_nome(e.cliente)
    ...
    e.credito_usado = float(usado_antes + consumir)

    mov = CreditoMovimento(
        cliente_id=cli.id, tipo="debito", valor=float(consumir),
        referencia=f"Entrega #{e.id}", entrega_id=e.id
    )
    db.session.add(mov)
    db.session.flush()
    e.credito_mov_id = mov.id

    # status/pagamento default
    if e.credito_usado >= float(valor):
        e.status_pagamento = "pago"
        if not (e.pagamento or "").strip(): e.pagamento = "Crédito"
        if not (e.recebido_por or "").strip(): e.recebido_por = "Crédito automático"
    else:
        if not (e.status_pagamento or "").strip(): e.status_pagamento = "pendente"
...
def desfazer_consumo_credito_da_entrega(entrega_id: int) -> Decimal:
    """
    Estorna TODO o crédito usado nesta entrega de volta ao saldo do cliente.
    Não altera status_pagamento/pagamento.
    """
    e = Entrega.query.get(entrega_id)
    if not e: return Decimal("0.00")
    ...
    mov_estorno = CreditoMovimento(
        cliente_id=cli.id, tipo="credito", valor=float(usado),
...
def consumo_total_do_credito(credito_id: int) -> float:
    total = (
        ...
        .filter(CreditoMovimento.credito_id == credito_id, CreditoMovimento.tipo == "debito")
        .scalar()
        or 0.0
    )
...
def cliente_login():
    if request.method == 'POST':
        ...
        senha = request.form.get('senha') or ''
        cli = Cliente.query.filter(func.lower(Cliente.username)==user).first()
...
def cliente_cadastro():
    if request.method == 'POST':
        ...
        if Cliente.query.filter(func.lower(Cliente.username)==username).first():
            flash('Usuário já existe.')
            return redirect(url_for('cliente_cadastro'))
        cli = Cliente(nome=nome, username=username)
        cli.set_senha(senha)
        db.session.add(cli)
        db.session.commit()
        flash('Cadastro criado! Faça login.')
        return redirect(url_for('cliente_login'))
    return render_template('cliente_cadastro.html')

@app.route('/cliente_logout')
def cliente_logout():
    session.pop('is_cliente', None)
...
def meu_credito():
    ...
    # usa template se existir; senão, fallback
    try:
        return render_template("meu_credito.html", cli=cli, movs=movs, to_brasilia=to_brasilia)
    except Exception:
        return render_template_string("""
<!doctype html><meta charset="utf-8"><title>Meu Crédito</title>
<h2>Olá, {{cli.nome}}!</h2>
<p>Saldo: <b>R$ {{ '%.2f'|format(cli.saldo_atual) }}</b></p>
...
def clonar_entrega(id):
    ...
    nova = Entrega(
        ...
        data_envio=datetime.utcnow(), data_atribuida=None, cooperado_id=None,
        status='pendente', status_pagamento='pendente',
        pagamento=e.pagamento, recebido_por=None, cliente_id=e.cliente_id
    )
    db.session.add(nova)
    db.session.commit()
    flash(f'Entrega #{e.id} clonada em #{nova.id}.')
...
def cadastrar_entrega():
    ...
    if request.method == 'POST':
        ...
        # tenta vincular ao Cliente
        cliente_id_form = request.form.get('cliente_id', type=int)
        cli = Cliente.query.get(cliente_id_form) if cliente_id_form else _find_cliente_by_nome(cliente_nome)

        entrega = Entrega(
            cliente=cliente_nome, bairro=bairro, valor=valor,
            data_envio=datetime.utcnow(), status_pagamento='pendente', status='pendente',
            pagamento=pagamento, cliente_id=(cli.id if cli else None)
        )
        if cooperado_id:
        ...
        # Consome crédito se a forma usar crédito
        try:
...
def editar_entrega(id):
    ...
    if request.method == 'POST':
        if is_admin:
            # 1) Estorna consumo anterior (se houve)
            try:
                desfazer_consumo_credito_da_entrega(entrega.id)
            except Exception as ex:
                current_app.logger.exception("Falha ao estornar crédito antes de editar %s: %s", entrega.id, ex)
            ...
            cliente_id_form = request.form.get('cliente_id', type=int)
            cli = Cliente.query.get(cliente_id_form) if cliente_id_form else _find_cliente_by_nome(novo_cliente_nome)
            ...
            # 3) Reaplica consumo SEM depender do select (usa sempre saldo)
            try:
                consumir_credito_em_entrega(entrega.id)
            except Exception as ex:
                current_app.logger.exception("Falha ao consumir crédito pós edição %s: %s", entrega.id, ex)
    ...
    return (render_template('editar_entrega.html', entrega=entrega, cooperados=cooperados)
            if is_admin else render_template('editar_entrega_cooperado.html', entrega=entrega))
...
def excluir_entrega(id):
    ...
    try:
        # 2) Desacopla vínculos de movimentos -> entrega_id NULL
        db.session.execute(text("UPDATE credito_movimento SET entrega_id = NULL WHERE entrega_id = :eid"), {"eid": id})
        db.session.execute(text("UPDATE entrega SET credito_mov_id = NULL WHERE id = :eid"), {"eid": id})
        ...
        db.session.rollback()
        flash('Não foi possível excluir: vínculos de crédito ativos.', 'danger')
...
def estatisticas_cooperado():
    ...
    for nome, dct in mapa_coop.items():
        ...
        ranking_cooperados.append({
            "nome": nome, "qtd": dct["qtd"], "total_valor": round(dct["total"], 2), "percent": percent
        })
    ...
    return render_template(
        ...
        cooperados=cooperados, cooperado_id=cooperado_id,
        data_inicio=data_inicio, data_fim=data_fim, status_pagamento=status_pagamento, cliente=cliente,
...
def estatisticas_cooperado_exportar_xlsx():
    ...
    for nome, d in soma_por_coop.items():
        ...
        linhas.append({"Cooperado": nome, "Qtd Entregas": d["qtd"], "Valor Total (R$)": round(d["total"], 2), "% do Total": round(percent, 1)})
    ...
    with pd.ExcelWriter(output, engine='xlsxwriter') as writer:
        ...
        ws.merge_range(0, 0, 0, last_col, titulo, writer.book.add_format({
            'bold': True, 'font_size': 14, 'align': 'center', 'valign': 'vcenter', 'font_color': '#003399'
        }))
        widths = [28, 14, 18, 12]
...
def clientes():
    ...
    if request.method == 'POST':
        ...
        existe_nome = Cliente.query.filter(func.lower(Cliente.nome) == nome.lower()).first()
        if existe_nome:
            flash('Já existe um cliente com esse nome.')
            return redirect(url_for('clientes'))

        if username:
            existe_user = Cliente.query.filter(func.lower(Cliente.username) == username).first()
            if existe_user:
                flash('Usuário de login já existe para outro cliente.')
                return redirect(url_for('clientes'))
...
def editar_cliente(id):
    ...
    if username:
        existe_user = Cliente.query.filter(
            func.lower(Cliente.username) == username,
            Cliente.id != id
        ).first()
        if existe_user:
            if request.headers.get('X-Requested-With') == 'fetch':
                return jsonify(ok=False, error='Usuário de login já utilizado por outro cliente.'), 400
            flash('Usuário de login já utilizado por outro cliente.')
            return redirect(url_for('clientes'))

    cl.nome = nome
    cl.telefone = telefone or None
    cl.bairro_origem = bairro_origem or None
    cl.endereco = endereco or None
    cl.username = username or None
    if senha:
        cl.set_senha(senha)
...
def exportar_clientes():
    ...
    for c in Cliente.query.order_by(Cliente.nome).all():
        ...
        rows.append([
            ...
            c.bairro_origem,
            c.endereco,
            float(c.saldo_atual or 0.0),
            br_date_ymd(s.get("ultimo")) if s else "",
            int((s or {}).get("qtd") or 0)
        ])

    df_out = pd.DataFrame(rows, columns=[
        "ID", "Nome", "Telefone", "Bairro", "Endereco", "SaldoAtual", "UltimoUso", "TotalPedidos"
    ])
...
def importar_clientes():
    ...
    col_user   = colget('usuario', 'username', 'login')
    ...
    for i, row in df_in.iterrows():
        try:
            ...
            user   = str(row.get(col_user) or '').strip().lower() if col_user else None
            ...
            if cl:
                ...
                if user:
                    # não mexe em senha por import
                    outro_user = Cliente.query.filter(
                        func.lower(Cliente.username) == user,
                        Cliente.id != cl.id
                    ).first()
                    if not outro_user:
                        cl.username = user
    ...
    try:
        ...
        db.session.rollback()
        current_app.logger.exception("Erro ao salvar no banco durante importação de clientes")
...
def lista_espera_reordenar():
    ...
    try:
        ...
        db.session.rollback()
        current_app.logger.error(f"Reordenar fila falhou: {e}")
        return ("", 500)
...
# ====== BOOTSTRAP BANCO / MIGRAÇÕES SIMPLES ======
def criar_bd():
    with app.app_context():
        ...
        ddl_cmds = [
            ...
            "ALTER TABLE cliente ADD COLUMN IF NOT EXISTS saldo_atual DOUBLE PRECISION DEFAULT 0",
            ...
            "ALTER TABLE entrega ADD COLUMN IF NOT EXISTS credito_usado DOUBLE PRECISION DEFAULT 0",
            "ALTER TABLE entrega ADD COLUMN IF NOT EXISTS credito_mov_id INTEGER",

            # FKs (Postgres - ignorado no SQLite)
            (
                "DO $$ BEGIN "
        ...
        idx_cmds = [
            ...
            "CREATE INDEX IF NOT EXISTS idx_cliente_username_lower ON cliente ((lower(username)))"
        ]
        for s in idx_cmds:
            try:
                db.session.execute(text(s))
