...
def local_date_window_to_utc_range(local_date: date):
    ...
    return inicio_brasil.astimezone(pytz.utc).replace(tzinfo=None), fim_brasil.astimezone(pytz.utc).replace(tzinfo=None)

def month_range_utc(local_date: date):
    first = local_date.replace(day=1)
    next_first = first.replace(year=first.year+1, month=1, day=1) if first.month==12 else first.replace(month=first.month+1, day=1)
...
def parse_local_datetime_to_utc_naive(data_str: str):
    ...
    return BRAZIL_TZ.localize(dt_local_naive).astimezone(pytz.utc).replace(tzinfo=None)
...
# ====== Normalização forte para nome de cliente ======
...
def normalize_letters_key(s: str) -> str:
    ...
    1) tira acentos
    2) lowercase
    3) remove números e pontuação (fica só letras e espaço)
    4) compacta espaços
    """
    if not s: return ''
...
def br_date_ymd(dt_utc_naive: datetime) -> str:
    """Converte UTC naive para data local Brasil em 'YYYY-MM-DD'."""
...
def verifica_feriado(data_ref=None):
    ...
    if key in MUNICIPAIS_NATAL: nomes.append(f"Feriado Municipal (Natal/RN) – {MUNICIPAIS_NATAL[key]}")
...
def _build_admin_url_from_referrer():
    ...
    try:
        p = urlparse(ref)
        if not p.path.endswith("/admin"): return None
        qs = parse_qs(p.query)
...
def _assert_entrega_do_cooperado(entrega: Entrega):
    ...
    if uid is None or session.get('is_admin'): abort(403)
    if entrega.cooperado_id != uid: abort(403)
...
def login():
    if request.method == 'POST':
        usuario = request.form.get('usuario'); senha = request.form.get('senha')
        if usuario and usuario.lower() == 'coopex':
            if senha == '05062721':
                session['user_id'] = 0; session['user_nome'] = 'Coopex'; session['is_admin'] = True
            ...
            if cooperado and cooperado.check_senha(senha):
                session['user_id'] = cooperado.id; session['user_nome'] = cooperado.nome; session['is_admin'] = False
...
def admin():
    ...
    entregas_all = query.options(joinedload(Entrega.cooperado)).order_by(Entrega.data_envio.desc()).all()
    ...
    total_mes = Entrega.query.filter(Entrega.data_envio >= mes_ini_utc, Entrega.data_envio <= mes_fim_utc).count()
    ano_ini_utc, ano_fim_utc = year_range_utc(hoje)
    total_ano = Entrega.query.filter(Entrega.data_envio >= ano_ini_utc, Entrega.data_envio <= ano_fim_utc).count()
    ...
    tem_pendente = Entrega.query.filter(
        Entrega.data_envio >= inicio_dia_utc, Entrega.data_envio <= fim_dia_utc,
        (Entrega.status_pagamento == None) | (Entrega.status_pagamento.ilike('pendente'))
    ).count() > 0

    lista_espera = ListaEspera.query.order_by(ListaEspera.pos.asc().nulls_last(), ListaEspera.created_at.asc().nulls_last()).all()
    ...
    return render_template(
        'admin.html', entregas=entregas, cooperados=cooperados,
...
def clonar_entrega(id):
    ...
    nova = Entrega(cliente=e.cliente, bairro=e.bairro, valor=e.valor, data_envio=datetime.utcnow(),
                   data_atribuida=None, cooperado_id=None, status='pendente', status_pagamento='pendente',
...
def painel_cooperado():
    if session.get('user_id') is None or session.get('is_admin'): return redirect(url_for('login'))
    ...
    if inicio:
        ...
        query = query.filter(Entrega.data_envio >= local_date_window_to_utc_range(di)[0])
    if fim:
        df = datetime.strptime(fim, "%Y-%m-%d").date()
        query = query.filter(Entrega.data_envio <= local_date_window_to_utc_range(df)[1])
    ...
    return render_template('painel_cooperado.html', entregas=entregas, total_geral=total_geral,
                           ...
                           to_brasilia=to_brasilia, status_pgto=status_pgto)
...
def cadastrar_cooperado():
    ...
    if request.method == 'POST':
        ...
        if Cooperado.query.filter(func.lower(Cooperado.nome) == (nome or '').lower()).first():
            ...
            c = Cooperado(nome=nome); c.set_senha(senha); db.session.add(c); db.session.commit()
            flash('Cooperado cadastrado!'); return redirect_back_to_admin()
...
def clientes():
    ...
    if request.method == 'POST':
        ...
        if not nome:
            flash('Informe o nome do cliente.'); return redirect(url_for('clientes'))
        existe = Cliente.query.filter(func.lower(Cliente.nome) == nome.lower()).first()
        if existe:
            flash('Já existe um cliente com esse nome.'); return redirect(url_for('clientes'))
        ...
        flash('Cliente cadastrado!'); return redirect(url_for('clientes'))

    # ===== Estatística robusta de uso por normalização forte =====
    # Agrega por string original -> depois consolida por chave normalizada
    aggs = (
        db.session.query(
    ...
    # Monta lista de clientes com ultimo_ymd e total_pedidos
    lista = []
...
def editar_cliente(id):
    ...
    if existe:
        ...
        flash('Já existe outro cliente com esse nome.'); return redirect(url_for('clientes'))

    cl.nome = nome; cl.telefone = telefone; cl.bairro_origem = bairro_origem; cl.endereco = endereco or None
    db.session.commit()

    # Recalcula estatística com normalização forte (para retorno AJAX)
    if request.headers.get('X-Requested-With') == 'fetch':
        k_full = normalize_letters_key(cl.nome or '')
        k_first = normalize_first_token(cl.nome or '')
        # busca todas as variações de Entrega agrupadas
        aggs = (
            db.session.query(
                Entrega.cliente.label('cli'),
                func.count(Entrega.id).label('qtd'),
        ...
        for row in aggs:
            raw = (row.cli or '')
            if normalize_letters_key(raw) == k_full or (not k_full and normalize_first_token(raw) == k_first) or (k_first and normalize_first_token(raw) == k_first):
                tot += int(row.qtd or 0)
                if row.ultimo and (ultimo is None or row.ultimo > ultimo): ultimo = row.ultimo
...
def excluir_cliente(id):
    ...
    if request.headers.get('X-Requested-With') == 'fetch': return ("", 204)
    flash('Cliente excluído.'); return redirect(url_for('clientes'))
...
def cadastrar_entrega():
    ...
    if request.method == 'POST':
        cliente = request.form.get('cliente'); bairro = request.form.get('bairro')
        valor = float(request.form.get('valor')); cooperado_id = request.form.get('cooperado_id')
        pagamento = request.form.get('pagamento')
        entrega = Entrega(cliente=cliente, bairro=bairro, valor=valor, data_envio=datetime.utcnow(),
                          status_pagamento='pendente', status='pendente', pagamento=pagamento)
        if cooperado_id:
            entrega.cooperado_id = int(cooperado_id); entrega.data_atribuida = datetime.utcnow()
        db.session.add(entrega)
        if cooperado_id: ListaEspera.query.filter_by(cooperado_id=int(cooperado_id)).delete()
        db.session.commit(); flash('Entrega cadastrada!'); return redirect_back_to_admin()
...
def agendar_entrega():
    ...
    if request.method == 'POST':
        cliente = request.form.get('cliente'); bairro = request.form.get('bairro')
        valor = float(request.form.get('valor')); data_str = request.form.get('data')
        status_entrega = request.form.get('status_entrega'); status_pagamento = request.form.get('status_pagamento')
        cooperado_id = request.form.get('cooperado_id'); pagamento = request.form.get('pagamento')
        data_envio = parse_local_datetime_to_utc_naive(data_str)
        entrega = Entrega(cliente=cliente, bairro=bairro, valor=valor, data_envio=data_envio,
        ...
        db.session.add(entrega)
        if cooperado_id: ListaEspera.query.filter_by(cooperado_id=int(cooperado_id)).delete()
        db.session.commit(); flash('Entrega agendada!'); return redirect_back_to_admin()
...
def editar_entrega(id):
    ...
    if not is_admin and entrega.cooperado_id != session['user_id']:
        flash("Acesso não permitido."); return redirect(url_for('painel_cooperado'))

    if request.method == 'POST':
        if is_admin:
            entrega.cliente = request.form.get('cliente'); entrega.bairro = request.form.get('bairro')
            entrega.valor = float(request.form.get('valor')); novo_coop_id = request.form.get('cooperado_id')
            if novo_coop_id:
                novo_coop_id = int(novo_coop_id)
                if entrega.cooperado_id != novo_coop_id:
                    entrega.cooperado_id = novo_coop_id; entrega.data_atribuida = datetime.utcnow()
            ...
            db.session.commit(); flash('Entrega atualizada!'); return redirect_back_to_admin()
        else:
            entrega.status_pagamento = (request.form.get('status_pagamento') or entrega.status_pagamento or 'pendente').lower()
            entrega.status = request.form.get('status') or entrega.status
            entrega.recebido_por = request.form.get('recebido_por')
            db.session.commit(); flash('Entrega atualizada!'); return redirect(url_for('painel_cooperado'))

    return render_template('editar_entrega.html', entrega=entrega, cooperados=cooperados) if is_admin \
           else render_template('editar_entrega_cooperado.html', entrega=entrega)

@app.route('/excluir_entrega/<int:id>', methods=['POST'])
def excluir_entrega(id):
    if not session.get('is_admin'): return redirect(url_for('login'))
    entrega = Entrega.query.get_or_404(id); db.session.delete(entrega); db.session.commit()
    flash('Entrega excluída.'); return redirect_back_to_admin()
...
def excluir_cooperado(id):
    ...
    ListaEspera.query.filter_by(cooperado_id=c.id).delete()
    db.session.delete(c); db.session.commit()
    flash('Cooperado excluído.'); return redirect_back_to_admin()

# ========= BOTÕES RÁPIDOS =========
@app.post('/entregas/<int:id>/marcar-pagamento')
def marcar_pagamento(id):
    if not session.get('is_admin'): return redirect(url_for('login'))
    e = Entrega.query.get_or_404(id); e.status_pagamento = "pago"; db.session.commit()
    return redirect_back_to_admin()

@app.post('/entregas/<int:id>/marcar-entregue')
def marcar_entregue(id):
    if not session.get('is_admin'): return redirect(url_for('login'))
    e = Entrega.query.get_or_404(id); e.status = "entregue"; db.session.commit()
    return redirect_back_to_admin()

# ========= JSON COOPERADO =========
@app.post('/cooperado/toggle_pagamento/<int:id>')
def toggle_pagamento(id):
    e = Entrega.query.get_or_404(id); _assert_entrega_do_cooperado(e)
    atual = (e.status_pagamento or 'pendente').lower()
    e.status_pagamento = 'pago' if atual != 'pago' else 'pendente'
    db.session.commit(); return jsonify(ok=True, status_pagamento=e.status_pagamento)

@app.post('/cooperado/marcar_entregue/<int:id>')
def cooperado_marcar_entregue(id):
    e = Entrega.query.get_or_404(id); _assert_entrega_do_cooperado(e)
    payload = request.get_json(silent=True) or {}; recebido_por = (payload.get('recebido_por') or '').strip()
    if not recebido_por: return jsonify(ok=False, error='Campo "recebido_por" é obrigatório.'), 400
    e.status = 'recebido'; e.recebido_por = recebido_por; db.session.commit(); return jsonify(ok=True)

# ====== ESTATÍSTICAS (ADMIN) ======
@app.route('/estatisticas_cooperado')
def estatisticas_cooperado():
    if not session.get('is_admin'): return redirect(url_for('login'))
    # ... (sem mudanças; mantendo sua versão anterior) ...
    # (para poupar espaço aqui, deixei idêntico ao que você enviou; se quiser eu colo de novo)
    # OBS: nada disso interfere nos clientes.
    # --- começo do trecho original ---
    cooperados = Cooperado.query.order_by(Cooperado.nome).all()
    cooperado_id = request.args.get('cooperado_id', 'todos')
    ...
    for e in entregas:
        ...
        mapa_coop[nm]["qtd"] += 1; mapa_coop[nm]["total"] += float(e.valor or 0); total_geral_periodo += float(e.valor or 0)

    ranking_cooperados = []
    for nome, dct in mapa_coop.items():
        percent = (dct["total"] / total_geral_periodo * 100.0) if total_geral_periodo > 0 else 0.0
        ranking_cooperados.append({"nome": nome, "qtd": dct["qtd"], "total_valor": round(dct["total"], 2), "percent": percent})
    ...
    for e in entregas:
        ...
        if cl and cl.bairro_origem: cont_bairros_origem[(cl.bairro_origem or '').strip()] += 1
    ranking_bairros_origem = [{"bairro": (b or 'Não informado'), "qtd": q} for b, q in cont_bairros_origem.most_common()]
    ranking_pgto = [{"forma": f, "qtd": q} for f, q in cont_pgto.most_common()]

    soma_por_cliente = defaultdict(lambda: {"qtd": 0, "total": 0.0})
    for e in entregas:
        if e.cliente: soma_por_cliente[e.cliente]["qtd"] += 1; soma_por_cliente[e.cliente]["total"] += float(e.valor or 0)
    ranking_clientes = [{"cliente": c, "qtd": d["qtd"], "total": round(d["total"], 2)} for c, d in sorted(soma_por_cliente.items(), key=lambda kv: kv[1]["total"], reverse=True)]
    ...
    estatisticas = {"total": total, "pagas": pagas, "pendentes": pendentes, "total_valor": total_valor, "ticket_medio": ticket_medio,
                    "dia_top": dia_top, "hora_pico": hora_pico, "pgto_top": pgto_top}

    por_ano_total = defaultdict(float); por_ano_qtd = defaultdict(int)
    for e in entregas:
        dt_local = to_brasilia(e.data_envio); 
        if not dt_local: continue
        ano_local = dt_local.year
        if ano_local < 2025: continue
        por_ano_qtd[ano_local] += 1; por_ano_total[ano_local] += float(e.valor or 0)
    ultimo_ano = max(por_ano_total.keys() | por_ano_qtd.keys()) if por_ano_total else max(2025, datetime.now(BRAZIL_TZ).year)
    chart_ano_labels = list(range(2025, ultimo_ano + 1))
    chart_ano_totais = []; chart_ano_qtd = []; chart_ano_ticket = []
    for y in chart_ano_labels:
        tot = float(por_ano_total.get(y, 0.0)); qtd = int(por_ano_qtd.get(y, 0)); tkt = (tot / qtd) if qtd else 0.0
        chart_ano_totais.append(round(tot, 2)); chart_ano_qtd.append(qtd); chart_ano_ticket.append(round(tkt, 2))

    return render_template('estatisticas_cooperado.html',
        cooperados=cooperados, cooperado_id=cooperado_id, data_inicio=data_inicio, data_fim=data_fim,
        status_pagamento=status_pagamento, cliente=cliente, estatisticas=estatisticas,
        ranking_cooperados=ranking_cooperados, ranking_bairros=ranking_bairros, ranking_bairros_origem=ranking_bairros_origem,
        ranking_pgto=ranking_pgto, ranking_clientes=ranking_clientes, horas_pico_top3=horas_pico_top3,
        chart_entregas_labels=chart_entregas_labels, chart_entregas_values=chart_entregas_values,
        chart_faturamento_labels=chart_faturamento_labels, chart_faturamento_values=chart_faturamento_values,
        periodo_legivel=periodo_legivel, chart_ano_labels=chart_ano_labels, chart_ano_totais=chart_ano_totais,
        chart_ano_qtd=chart_ano_qtd, chart_ano_ticket=chart_ano_ticket
    )
    # --- fim do trecho original ---

# ====== EXPORTAÇÃO DETALHADA (sem HORA) ======
...
def exportar_xlsx():
    ...
    query = Entrega.query
    if cooperado_id != 'todos': query = query.filter(Entrega.cooperado_id == int(cooperado_id))
    if cliente:
        like = f"%{cliente.lower()}%"; query = query.filter(func.lower(Entrega.cliente).like(like))
    if data_inicio:
        di = datetime.strptime(data_inicio, "%Y-%m-%d").date()
        query = query.filter(Entrega.data_envio >= local_date_window_to_utc_range(di)[0])
    if data_fim:
...
def exportar_clientes():
    ...
    # (reusa as estatísticas robustas para exportar)
    aggs = (
        db.session.query(
            Entrega.cliente.label('cli'),
            func.count(Entrega.id).label('qtd'),
    ...
    with pd.ExcelWriter(output, engine='xlsxwriter') as writer:
        ...
        for i, w in enumerate(widths[:len(df.columns)]): ws.set_column(i, i, w)
    output.seek(0)
...
def importar_clientes():
    ...
    if not f:
        if request.headers.get('X-Requested-With') == 'fetch': return jsonify(ok=False, error="Envie um arquivo .xlsx"), 400
        flash("Envie um arquivo .xlsx"); return redirect(url_for('clientes'))

    try:
        df = pd.read_excel(f)
    except Exception:
        if request.headers.get('X-Requested-With') == 'fetch': return jsonify(ok=False, error="Não consegui ler o XLSX."), 400
        flash("Não consegui ler o XLSX."); return redirect(url_for('clientes'))
    ...
    col_id = colget('id'); col_nome = colget('nome'); col_tel = colget('telefone','phone')
    col_bairro = colget('bairro','bairro_origem'); col_endereco = colget('endereco','endereço')

    adicionados = 0; atualizados = 0; erros = 0
    for _, row in df.iterrows():
        try: rid = int(row[col_id]) if (col_id and not pd.isna(row[col_id])) else None
        ...
        if not nome: erros += 1; continue

        cl = Cliente.query.get(rid) if rid else None
        if not cl:
            cl = Cliente.query.filter(func.lower(Cliente.nome) == nome.lower()).first()
        if cl:
            cl.nome = nome; cl.telefone = tel; cl.bairro_origem = bairro; cl.endereco = ender or None; atualizados += 1
        else:
            db.session.add(Cliente(nome=nome, telefone=tel, bairro_origem=bairro, endereco=ender or None)); adicionados += 1

    try:
        db.session.commit()
    except Exception:
        db.session.rollback()
        if request.headers.get('X-Requested-With') == 'fetch': return jsonify(ok=False, error="Erro ao salvar no banco."), 500
        flash("Erro ao salvar no banco."); return redirect(url_for('clientes'))
...
def lista_espera_add():
    if not session.get('is_admin'): return redirect(url_for('login'))
    cooperado_id = request.form.get('cooperado_id'); nome_form = (request.form.get('nome') or '').strip()
    if not cooperado_id and not nome_form:
        flash('Selecione um cooperado ou informe um nome.'); return redirect_back_to_admin()

    if cooperado_id:
        coop = Cooperado.query.get(int(cooperado_id))
        if not coop: flash('Cooperado inválido.'); return redirect_back_to_admin()
        if ListaEspera.query.filter_by(cooperado_id=coop.id).first():
            flash('Este cooperado já está na fila de espera.'); return redirect_back_to_admin()
        max_pos = db.session.query(func.max(ListaEspera.pos)).scalar() or 0
        db.session.add(ListaEspera(cooperado_id=coop.id, nome=coop.nome, pos=max_pos+1, created_at=datetime.utcnow()))
        db.session.commit(); flash('Cooperado adicionado à lista de espera.'); return redirect_back_to_admin()

    if ListaEspera.query.filter(func.lower(ListaEspera.nome) == nome_form.lower()).first():
        flash('Este nome já está na fila de espera.'); return redirect_back_to_admin()
    max_pos = db.session.query(func.max(ListaEspera.pos)).scalar() or 0
    db.session.add(ListaEspera(nome=nome_form, cooperado_id=None, pos=max_pos+1, created_at=datetime.utcnow()))
    db.session.commit(); flash('Nome adicionado à lista de espera.'); return redirect_back_to_admin()

@app.route('/lista_espera/remove/<int:id>', methods=['POST'])
def lista_espera_remove(id):
    if not session.get('is_admin'): return redirect(url_for('login'))
    item = ListaEspera.query.get_or_404(id); db.session.delete(item); db.session.commit()
    flash('Removido da lista de espera.'); return redirect_back_to_admin()

@app.route('/lista_espera/reordenar', methods=['POST'])
def lista_espera_reordenar():
    if not session.get('is_admin'): return ("", 403)
    data = request.get_json(silent=True) or {}; ordem = data.get('ordem') or []
    try:
        ...
        db.session.rollback(); app.logger.error(f"Reordenar fila falhou: {e}"); return ("", 500)
